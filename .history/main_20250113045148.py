...
class text_reader:
    ...
    def extract_text(self) -> str:
        """Extract text from Text with proper error handling."""
        ...
        try:
            ...
            logger.error("Error extracting text from text: %s", str(e))
            raise
...
class ThreatIntelModel:
    ...
    def __init__(
        self, vocab_size: int = 5000, embedding_dim: int = 100, input_length: int = 200
    ):
...
def main():
    ...
    try:
        ...
        config = IOCConfig()
        pdf_extractor = PDFExtractor("data/raw_reports")
        ...
        try:
            ...
            logger.info(
                "Successfully processed document. Found %d IOCs.", len(results_df)
            )
