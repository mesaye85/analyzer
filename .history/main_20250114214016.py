...
class IOCValidator:
    ...
    def validate_domain(domain: str) -> bool:
        ...
        return all(len(x) <= 63 for x in domain.split(".")) and bool(
            allowed.match(domain)
        )
...
def extract_iocs(text: str) -> Dict[str, List[str]]:
    ...
    for ioc_type, pattern in patterns.items():
        ...
        if ioc_type == "ips":
            iocs[ioc_type] = list(
                set(ip for ip in matches if IOCValidator.validate_ip(ip))
            )
        elif ioc_type == "domains":
            iocs[ioc_type] = list(
                set(
                    domain for domain in matches if IOCValidator.validate_domain(domain)
                )
            )
        else:
...
def main():
    ...
    try:
        ...
        if results:
            ...
            # Display IOC extraction metrics
            ioc_metrics = {
            ...
            print(metrics_display.format_metrics(ioc_metrics))
            logger.info(
                "IOC Extraction Metrics:\n"
                + metrics_display.format_metrics(ioc_metrics)
            )
